...
//				Si on a fait toutes les pi�ces on tourne d'un quart de tour la premi�re et on recommence
				if (piecesToutesFaites) {// Si on a fait toutes les pi�ces sans trouver de solutions
					...
					//Tant qu'on a pas plac� une pi�ce correctement au dessus
					while (bienPlacee(board.getPositions().get(this.ordrePlacement[n]))) {
						cptTours = 0;
						//On fait tourner la pi�ce n pour voir si il n'existe pas une autre solution
						while (bienPlacee(board.getPositions().get(this.ordrePlacement[n])) || cptTours < 2) {
							board.getPositions().get(this.ordrePlacement[n]).tourne(1);
							fen.refreshBoard();
							System.out.println(board.getPositions().get(this.ordrePlacement[n]).toString()+"\ta tourn�\tcompteur de tours : "+cptTours+"\tValeur de n : "+n);
						}
						...
						//Sinon on retire la piece et on essaye de trouver une autre solution avec la pr�c�dente
						if (cptTours > 2) {//Sinon on enl�ve la pi�ce et on essaye avec la pr�c�dente
	...
	*/
	...
	public void resoudreAide(int n, int nPrecedent, int nbTours, int nbPiecesTestees, boolean avance, ArrayList<InstanceBoard> boardsFaux) throws InterruptedException{
		...
		nettoieDoublon(boardsFaux);
		System.out.println("n : "+n+" nPr�c�dent : "+nPrecedent+" nbTours : "+nbTours+" nbPiecesTestees : "+nbPiecesTestees+"/"+board.getPool().getPool().size()+" avance : "+avance);
		if (n < 0) {
		...
		if (nbPiecesTestees == board.getPool().getPool().size()) {
			...
			boardsFaux.add(new InstanceBoard(board));System.out.println("Board ajout� !");
			System.out.println(board.toString(board.getInstance()));
			resoudreAide(n - 1, n, nbTours, 0, false, boardsFaux);
		}
...
//				}
				...
				resoudreAide(n - 1, n, 0, nbPiecesTestees, true, boardsFaux);
			}
		}
		if (n > nPrecedent) {
	...
	public void nettoieDoublon(ArrayList<InstanceBoard> boardsFaux){
		...
		for (int i = 0; i < boardsFaux.size(); i++) {
			cpt = 0;
			for (int j = 0; j < boardsFaux.size(); j++) {
				if (aEteUneInstance(boardsFaux.get(i), boardsFaux)) {
					cpt++;
					indicesANettoyer.add(j);
				}
			}
			if (cpt > 1) {
				System.out.println(indicesANettoyer.size());
				System.out.println(indicesANettoyer.get(1));
				for (int j = indicesANettoyer.size()-1; j > 1; j--) {
					boardsFaux.remove(indicesANettoyer.get(j));
				}
			}
		}
